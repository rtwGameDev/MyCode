...
int main()
{
    ...
    {
        DATE_TIME time("1", "1", "2004", "9", "7");
        Format(time);
        cout << getStringDATE_TIME(time);
    }
    {
        //DATE_TIME before("27", "3", "2024", "15", "30");
        //Format(before);
        //DATE_TIME after = getCurrentTime();
    ...
    }
    {
        ...
        TICKET* LIST_TICKET = new TICKET[soCho + 1];
        GTicketNumber(soDay, soDong, LIST_TICKET);
        for (int i = 1; i <= soDay; ++i) {
           for (int j = 1; j <= soDong; ++j) {
               cout << LIST_TICKET[index++].TICKET_CODE << " ";
           }
           cout << endl;
        }
    }
